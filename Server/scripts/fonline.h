...
#define ITEM_USER_DATA_SIZE          (100)
// Vars
...
// „ерез нативный метод Sprite** GameOptions::GetDrawingSprites( uint & count ) находишь весь список спрайтов, в этом списке находишь свой спрайт,
struct GameOptions
{
...
struct ProtoItem
{
    ...
    // Common
    const int    MagicPower;                  // 0,1,2,3 - 4 байта
    const uint8  Unused[ 96 ];                // от 4 до 99 байт
    // Armor, offset 100

    const uint   Armor_CrTypeMale;            // 100-103
    const uint   Armor_CrTypeFemale;          // 104-107
    const int    Armor_AC;                    // 108-111
    const uint   Armor_Perk;                  // 112-115
    const int    Armor_DRNormal;              // 116-119
    const int    Armor_DRLaser;               // 120-123
    const int    Armor_DRFire;                // 124-127
    const int    Armor_DRPlasma;              // 128-131
    const int    Armor_DRElectr;              // 132-135
    const int    Armor_DREmp;                 // 136-139
    const int    Armor_DRExplode;             // 140-143
    const int    Armor_DTNormal;              // 144-147
    const int    Armor_DTLaser;               // 148-151
    const int    Armor_DTFire;                // 152-155
    const int    Armor_DTPlasma;              // 156-159
    const int    Armor_DTElectr;              // 160-163
    const int    Armor_DTEmp;                 // 164-167
    const int    Armor_DTExplode;             // 168-171
    const uint8  Armor_Unused[ 28 ];          // 172-199
    // Weapon, offset 200
    const int    Weapon_DmgType[ 3 ];         // 200-211
    const uint   Weapon_Anim2[ 3 ];           // 212-223
    const int    Weapon_DmgMin[ 3 ];          // 224-235
    const int    Weapon_DmgMax[ 3 ];          // 236-247
    const uint16 Weapon_Effect[ 3 ];          // 248-253
    const bool   Weapon_Remove[ 3 ];          // 254-256
    const uint8  Weapon_Unused0[ 3 ];         // 257-259 = 3 + Add Cracker
    const uint   Weapon_ReloadAp;             // 260-263
    const int    Weapon_UnarmedCriticalBonus; // 264-267
    const uint   Weapon_CriticalFailture;     // 268-271
    const bool   Weapon_UnarmedArmorPiercing; // 272
    // mk2 +Add Cracker
    // lightning weapon
    const int16  Weapon_LightningRadius;      // 273-274
    const uint8  Weapon_LightningCountTarget; // 275
    // Mortira
    const uint8  Weapon_FlyingDistance;       // 276
    const uint8  BonusForOutOfCombat;         // 277
    // ++ Edit Cracker ++ old - const uint8  Weapon_Unused[ 27 ];
    const uint8  Weapon_Unused1[ 1 ];         // 278

    // Other mk2
    const uint8  CraftInfo;                   // 279
    const uint8  MaxRepairCount;              // 280

    // Ammo
    const int8   Ammo_AcMod;                  // 281
    const int8   Ammo_DrMod;                  // 282
    const uint8  Ammo_DmgMult;                // 283
    const uint8  Ammo_DmgDiv;                 // 284

    // Add weapon
    const uint8  Weapon_SecondPerk;           // 285
    const int8   Weapon_CriticalBonus;        // 286
    const int8   Weapon_BonusCritRoll;        // 287
    const int8   Reserved;                    // 288

    // Randomisable
    const uint8  Randomisable;                // 289

    // ?л¤ книг. Cracker+++
    const uint8  IsReadableBook;              // 290
    const uint8  BookAddPoint;                // 291
    const uint8  BookCountRead;               // 292
    const uint8  IsSkillBook;                 // 293
    const uint8  BookNumberParam;             // 294
    const uint8  BookNumber;                  // 295

    const uint8  BrokenCoefficient;           // 296
    const uint16 MaxHP;                       // 297-298
    const uint8  Weapon_NoBurst[ 3 ];         // 299-301
    const uint16 IndexCraftList;              // 302
    const int16  DefaultHP;                   // 304
    const bool   IsAttack;                    // 306
    const uint8  HexWeapon_Radius;            // 307

    // Ammo ext
    const uint8  Ammo_Perk;                   // 308
    const uint8  Ammo_DmgType;                // 309

    const uint8  Weapon_IgnoreAC;             // 310

    const uint8  Ammo_DTDiv;                  // 311

    const uint16 ReservedByte312;                 // 312

    const int16  Weapon_BonusToHit;
    // Other
    const uint8  UnusedEnd[ 185 ];                              // 316-499
...
struct NpcPlane
{
    ...
    union
    {
        ...
        struct
        {
            ...
            int    MinHp;                     // 9
            bool   IsGag;                     // 10
            uint16 GagHexX, GagHexY;          // 14
            uint16 LastHexX, LastHexY;        // 18

            uint8  hamberStage;               // 22
            uint16 hamberStageBestCoord[ 2 ]; // 32
        } const Attack;
        ...
        struct
        {
             ...
             */
            uint8  TypeWalkReason;
            uint   Transit_Id;             // јйди
...
struct Item
{
    ...
    const bool       ViewPlaceOnMap;
		  uint16 	 InitVersion;  // SDK: const int16      Reserved0
    // bool   IsExpansion;
    ...
    struct _Data
    {
        ...
        union
        {
            const char  Reserved[ 2 ];           // SDK
            const int16 CraftFlags;              // MK2
        };
		const uint UpgradeSlot[3];
		const uint StackUid;
		// User data, binded with 'bindfield' pragma
		union
		{
			const int UserData[ITEM_USER_DATA_SIZE / sizeof(int)];
			struct
			{
				uint CrafterId;
				uint8 WeaponBonus_MinDamage;
				uint8 WeaponBonus_MaxDamage;
				uint8 WeaponBonus_Critroll;
				uint8 WeaponBonus_ToHit;
				uint8 WeaponBonus_CritChance;
				uint8 WeaponBonus_AP;
			};
		};
    } Data;
	
	asIScriptObject* ScriptDescriptor;
    ...
    inline uint GetCost()
    {
        return ( Data.Cost == 0 ? Proto->Cost : Data.Cost ) * GetCount();
    }
...
struct Critter
{
    ...
    union
    {
        ...
        {
            uint   ToLeader_MapId;
            uint16 ToLeader_HexX;
            uint16 ToLeader_HexY;
        };
    };
	...
	union
    {
        ...
        const uint Reserved9[ 10 ];             // SDK - 40
    };
    ...
    union
    {
        ...
        struct
        {
            const uint8 _UserData[ CRITTER_USER_DATA_SIZE - ( 80 *  sizeof( BookData ) ) - sizeof( uint ) ];
            BookData    Book[ 80 ];
        };
    };
    // Npc data
    ...
    union      // Farm params 16 byte
    {
        struct // mk2
        {
            const bool IsFarmGenerated;
            const uint FarmUnicId;
            const uint FarmProtoMobId;
            // 7 byte free
        };
    ...
    union
    {
        ...
        {
            uint8 CountMerc;
            uint8 Reserved3byte[ 7 ];
            // 30
            uint  MercId[ 30 ];
            uint  HousScriptId[ 24 ];
    ...
    uint8                     get_SurplusPoint( uint ind )
    {
        if( ind < 80 )
        {
            return Book[ ind ].SurplusPoint;
        }
        return 80;
    }

    void set_SurplusPoint( uint ind, uint8 value )
    {
        if( ind < 80 )
        {
            Book[ ind ].SurplusPoint = value;
        }
    }

    uint8 get_CountReadBook( uint ind )
    {
        if( ind < 80 )
        {
            return Book[ ind ].CountRead;
        }
        return 80;
    }

    void set_CountReadBook( uint ind, uint8 value )
    {
        if( ind < 80 )
        {
            Book[ ind ].CountRead = value;
        }
    }

    void set_SetMaster( Critter& master )
    {
        if( master.CountMerc < 30 )
        {
            for( uint i = 0; i < master.CountMerc; i++ )
            {
                if( master.MercId[ i ] == this->Id )
                    return;
            }

            master.MercId[ master.CountMerc++ ] = Id;

            FOnline->CritterChangeParameter( *this, MERC_MASTER_ID );
            const_cast< int* >( Params )[ MERC_MASTER_ID ] = master.Id;

            FOnline->CritterChangeParameter( master, MASTER_COUNT_MERC );
            const_cast< int* >( master.Params )[ MASTER_COUNT_MERC ] = master.CountMerc;
        }
    }
    ...
    uint GetMercIds( CScriptArray* outArr )
    {
        if( outArr )
            ...
            {
                outArr->InsertLast( &MercId[ i ] );
            }
        return CountMerc;
    }

    void FreeMercSlot( uint index )
    {
        if( CountMerc > index )
        {
            for( uint i = index; i < CountMerc; i++ )
            {
                if( i == CountMerc - 1 )
                {
                    MercId[ i ] = 0;
            ...
            FOnline->CritterChangeParameter( *this, MASTER_COUNT_MERC );
            const_cast< int* >( Params )[ MASTER_COUNT_MERC ] = CountMerc;
        }
    }

    void DeleteMerc( Critter& merc )
    {
        {
            for( uint i = 0; i < CountMerc; i++ )
            {
            ...
            FOnline->CritterChangeParameter( merc, MERC_MASTER_ID );
            const_cast< int* >( merc.Params )[ MERC_MASTER_ID ] = 0;
        }
    }
    ...
    uint GetItemsWeight() const
    {
        ...
        {
            const Item* item = *it;
            if( !item->IsHidden() ) weight += item->GetWeight();
        }
        return weight;
    }
    ...
    uint GetItemsVolume() const
    {
        ...
        {
            const Item* item = *it;
            if( !item->IsHidden() ) volume += item->GetVolume();
        }
        return volume;
    }

    inline int Timeout( uint index )
    {
        return (uint) Params[ index ] > FOnline->FullSecond ? (uint) Params[ index ] - FOnline->FullSecond : 0;
    }
};
...
struct MapObject
{
    ...
    union
    {
        struct
        {
            ...
            int   ParamValue[ MAPOBJ_CRITTER_PARAMS ]; // 400
        } const MCritter;
...
typedef vector< SceneryToClient >::const_iterator SceneryToClientVecIt;
...
struct Map
{
    ...
    struct
    {
        ...
        const uint       FarmMapVersion;     // MK2

        const int        CityManagerIndex;   // MK2
        const bool       CityManager_IsNoob; // MK2

        const bool       FarmMapIsProtect;   // MK2
        const uint       MapDynamicIndex;    // MK2
        const uint       MapDynamicObjectCount;
        const uint       MapDynamicVersion;
        bool             IsExpansion;
        asIScriptObject* ScriptClass;
        // const char   ReservedMK2[ 1 ];
        const uint       Reserved[ 12 ];     // MK2

        // const uint   Reserved[ 20 ]; // SDK
    ...
    #ifdef __SERVER
    bool SetHexFlag( uint16 hx, uint16 hy, uint flag ) const { if( hy < GetMaxHexY() && hx < GetMaxHexX() ) { SETFLAG( const_cast< uint8* >( HexFlags )[ hy * GetMaxHexX() + hx ], flag );
                                                                                                              return true; } return false; }
    bool UnSetHexFlag( uint16 hx, uint16 hy, uint flag ) const { if( hy < GetMaxHexY() && hx < GetMaxHexX() ) { UNSETFLAG( const_cast< uint8* >( HexFlags )[ hy * GetMaxHexX() + hx ], flag );
                                                                                                                return true; } return false; }
    ...
    bool   IsHexItem( uint16 hx, uint16 hy )    const { return FLAG( HexFlags[ hy * GetMaxHexX() + hx ], FH_ITEM ); }
...
struct Location
{
    ...
    struct
    {
        ...
        const uint       FarmLocId;
        const uint       FarmLocPId;
        const uint       FarmLocVersion;
        const uint8      TypeLocation;

        bool             IsExpansion;
        asIScriptObject* ScriptClass;

        const uint       FractionId;
        const uint       FarmLocTimeEventId;
		const int        SlowProcess;
		const int        SlowProcessTick;
        const uint       Reserved3[ 49 ];
        const char       byte_Reserved3[ 3 ];


        // const uint   Reserved3[ 59 ]; // SDK
    } const Data;
...
inline void static_asserts()
{
    ...
    # ifdef __SERVER
    STATIC_ASSERT( offsetof( ProtoMap, HexFlags )              == 432  );
    # endif
