...
//Через нативный метод Sprite** GameOptions::GetDrawingSprites( uint & count ) находишь весь список спрайтов, в этом списке находишь свой спрайт, 
struct GameOptions 
{
...
struct ProtoItem
{
	...
	const int8    Weapon_BonusToHit;			// 288
    ...
    // Other
    const uint8  UnusedEnd[ 198 ];				// 302-499
...
struct Item
{
	...
	union
	{
		//const int16      CurrentHp; // MK2
		const int16      Reserved0; // SDK
	};
...
struct Critter
{
    ...
    const uint8  Profession; //Reserved21;
    const uint8  BagSize;
    struct
    {
        const uint ItemPid;
	...
	union
	{
		...
		struct
        {
			...
			uint8  MaxMerc;
			uint   MercId[ 90 ];
        } const PRSlaveTrade; //Profession=1
	...
	uint GetMercId(uint index) const
	{
		if(index<PRSlaveTrade.CountMerc) return PRSlaveTrade.MercId[index];
		return 0;
	}
	
	bool IsMercId(uint id) const
	{
		for(uint i=0;i<PRSlaveTrade.CountMerc;i++)
		{
			if(PRSlaveTrade.MercId[i]==id) return true;
		}
		return false;
	}
...
struct Map
{
    ...
    struct
    {
		...
		const char   ReservedMK2[ 2 ];
		const uint   Reserved[ 17 ]; // MK2
